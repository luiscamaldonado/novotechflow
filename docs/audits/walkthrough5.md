# Refactoring ProposalCalculations.tsx

## Tabla Resumen

| Componente / Archivo | Líneas antes | Nuevo archivo | Líneas después |
| :--- | :--- | :--- | :--- |
| **ProposalCalculations.tsx** | **796** | *(mismo archivo, refactorizado)* | **363** |
| ScenarioItemRow | — | `components/ScenarioItemRow.tsx` | **228** |
| ScenarioSidebar | — | `components/ScenarioSidebar.tsx` | **131** |
| ScenarioHeader | — | `components/ScenarioHeader.tsx` | **163** |
| `AcquisitionMode` + `ACQUISITION_OPTIONS` | *(inline en ProposalCalc)* | `lib/constants.ts` (+15 líneas) | — |

> [!NOTE]
> Se redujeron **433 líneas** del archivo principal (796 → 363). La lógica de DaaS (`savedMarginsRef`, `acquisitionModes`, `handleAcquisitionChange`, `isDaasMode`) se mantuvo en el componente principal porque depende de `totals`, `activeScenario`, `updateGlobalMargin` y `updateMargin` — moverlo a un hook separado no simplificaría nada.

## Qué se extrajo a cada componente

### 1. ScenarioItemRow (~228 líneas)
- El `<tr>` completo con todas las celdas: #item, nombre, cantidad, margen, unitario, total, botón diluir, botón eliminar
- Lógica inline de edición de celdas (`editingCell`, `onFocus`, `onChange`, `onBlur`)
- Renderizado condicional por modo (diluted / DaaS / normal) en columnas de margen y precio
- Bloque expandible de sub-ítems (`children`) con sus propias filas, inputs de cantidad, costos y botón de eliminar

### 2. ScenarioSidebar (~131 líneas)
- La columna `lg:col-span-3` completa
- Lista de escenarios con `AnimatePresence`
- Botones de clonar y eliminar por escenario
- Formulario inline para crear escenario nuevo (`isCreatingScenario` y `newScenarioName` ahora son estado local del componente)
- Estado vacío con ícono y mensaje

### 3. ScenarioHeader (~163 líneas)
- Barra superior del escenario activo
- Nombre editable del escenario (`editingScenarioName` y `scenarioNameInputRef` ahora son estado local)
- Input de margen global con buffer
- Selector de adquisición (VENTA / DaaS)
- Switcher de moneda COP / USD
- Botón "Pick de Items"

### 4. constants.ts (+15 líneas)
- `AcquisitionMode` type
- `ACQUISITION_OPTIONS` array

## Estado que se movió fuera del componente principal

| Estado | Antes | Ahora es local en |
| :--- | :--- | :--- |
| `isCreatingScenario` | ProposalCalculations | ScenarioSidebar |
| `newScenarioName` | ProposalCalculations | ScenarioSidebar |
| `editingScenarioName` | ProposalCalculations | ScenarioHeader |
| `scenarioNameInputRef` | ProposalCalculations | ScenarioHeader |

## Diff de ProposalCalculations.tsx — qué se eliminó
...
- [ScenarioItemRow.tsx](file:///d:/novotechflow/apps/web/src/pages/proposals/components/ScenarioItemRow.tsx)
- [ScenarioSidebar.tsx](file:///d:/novotechflow/apps/web/src/pages/proposals/components/ScenarioSidebar.tsx)
- [ScenarioHeader.tsx](file:///d:/novotechflow/apps/web/src/pages/proposals/components/ScenarioHeader.tsx)
- [constants.ts](file:///d:/novotechflow/apps/web/src/lib/constants.ts) (modificado)

## Verificación

> [!IMPORTANT]
> No se ejecutó `tsc` ni `pnpm` según las reglas del usuario. La UI debe comportarse idénticamente — no se cambió ninguna funcionalidad, nombres de props, ni lógica de negocio.
