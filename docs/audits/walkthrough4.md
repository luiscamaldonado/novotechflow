# Dashboard.tsx Decomposition — Walkthrough

## Summary Table

| Archivo | Antes | Después | Descripción |
|:---|:---:|:---:|:---|
| [Dashboard.tsx](file:///d:/novotechflow/apps/web/src/pages/Dashboard.tsx) | 1,005 | **405** | Solo imports, hook wiring, JSX de tabla y layout |
| [useDashboard.ts](file:///d:/novotechflow/apps/web/src/hooks/useDashboard.ts) | — | **411** | Todo el estado, filtros, billingCards, acciones de proposals/projections |
| [useProjections.ts](file:///d:/novotechflow/apps/web/src/hooks/useProjections.ts) | — | **102** | CRUD modal de proyecciones (save, delete, form state) |
| [ProjectionModal.tsx](file:///d:/novotechflow/apps/web/src/pages/dashboard/ProjectionModal.tsx) | — | **135** | JSX del modal de proyecciones |
| [BillingCards.tsx](file:///d:/novotechflow/apps/web/src/pages/dashboard/BillingCards.tsx) | — | **71** | JSX de las 6 tarjetas financieras |
| [constants.ts](file:///d:/novotechflow/apps/web/src/lib/constants.ts) | 113 | **144** | +31 líneas: STATUS_CONFIG, ALL_STATUSES, PROJECTION_STATUSES, ACQUISITION_CONFIG, formatCOP |

> [!NOTE]
> Dashboard.tsx quedó en 405 líneas en vez de 350 debido a que la tabla principal (filas de propuestas + proyecciones) es JSX puro y denso. No se puede reducir más sin extraer las filas de la tabla a componentes separados, lo cual fragmentaría excesivamente el código sin beneficio real.

---

## Diffs por Archivo

### 1. `useDashboard.ts` [NUEVO]
...
**Contenido extraído de Dashboard.tsx:**
- `computeMinSubtotal()` — usa `calculateScenarioTotals` del pricing-engine
- `DashboardRow` interface + `BillingCards` interface
- Todo el estado: `proposals`, `projections`, `loading`, `cloning`, `showFilters`, `searchTerm`, `statusFilters`, `subtotalMin`, `subtotalMax`
- `loadData()`, `loadProposals()`
- `proposalsWithSubtotals` (useMemo)
- `allRows` (useMemo) — fusión proposals + projections
- `filtered` (useMemo) — filtros de búsqueda/estado/subtotal
- `billingCards` (useMemo) — métricas financieras por trimestre
- Todas las acciones: `handleStatusChange`, `handleDateChange`, `handleClone`, `handleDelete`, `handleAcquisitionChange`, `handleProjectionAcquisitionChange`, `handleProjectionStatusChange`, `handleProjectionDateChange`
- `toggleStatusFilter`, `clearFilters`, `hasActiveFilters`
- Expone `setProjections` para que `useProjections` pueda hacer updates optimistas

---

### 2. `useProjections.ts` [NUEVO]
...
**Contenido extraído de Dashboard.tsx:**
- `ProjForm` interface (exportada para `ProjectionModal`)
- Estado del modal: `showProjectionModal`, `editingProjection`, `projForm`, `savingProjection`
- `openNewProjectionModal()`, `openEditProjectionModal()`
- `handleSaveProjection()` — create/update con API + optimistic update
- `handleDeleteProjection()` — delete con confirm + optimistic update
- Recibe `setProjections` del hook `useDashboard` para mantener el estado compartido

---

### 3. `ProjectionModal.tsx` [NUEVO]
...
**Contenido extraído:** Todo el JSX del modal `fixed inset-0 z-50` (líneas 889–1001 del original).

---

### 4. `BillingCards.tsx` [NUEVO]
...
**Contenido extraído:** Las 6 tarjetas financieras (líneas 518–573 del original).

---

### 5. `constants.ts` [MODIFICADO]
...
**Añadido:**
- `STATUS_CONFIG` — configuración de badges por estado
- `ALL_STATUSES` — lista ordenada de estados
- `PROJECTION_STATUSES` — estados válidos para proyecciones
- `ACQUISITION_CONFIG` — configuración de badges por tipo de adquisición
- `formatCOP()` — formateador de moneda COP

---

### 6. `Dashboard.tsx` [REESCRITO]
...
**Lo que queda:**
- Imports (12 líneas)
- Destructuring de `useDashboard()` + `useProjections()` (16 líneas)
- Loading spinner (5 líneas)
- Header JSX (26 líneas)
- `<BillingCards />` (1 línea)
- Search + Filters JSX (70 líneas)
- Tabla principal con filas de proyecciones y propuestas (270 líneas — JSX puro)
- `<ProjectionModal />` (9 líneas)

---

## Arquitectura de Comunicación entre Hooks

```mermaid
graph LR
    D["Dashboard.tsx"] --> UD["useDashboard()"]
    D --> UP["useProjections()"]
    UD -->|setProjections| UP
    UP -->|optimistic updates| UD
    UD -->|billingCards| BC["BillingCards"]
    UP -->|form state| PM["ProjectionModal"]
```

`useDashboard` posee el estado de `projections[]` y expone `setProjections`. `useProjections` lo recibe como argumento para hacer updates optimistas idénticos al código original.
...
- ✅ Mismo optimistic update pattern (no reload after save)
- ✅ Mismos props sin renombrar
- ✅ `computeMinSubtotal` usa `calculateScenarioTotals` del pricing-engine
- ✅ Constantes idénticas movidas a constants.ts
