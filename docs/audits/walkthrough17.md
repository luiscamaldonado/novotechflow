...
### [NEW] [exportDashboard.ts](file:///d:/novotechflow/apps/web/src/lib/exportDashboard.ts)

New module (239 lines) that generates a professionally styled Excel file from filtered Dashboard rows.

**Key features:**
- 13 columns: Usuario, Código, Cliente, Asunto, Estado, Adquisición, F. Cierre, F. Facturación, Moneda, Subtotal Min., USD Est., Última Actualización, Tipo
- Status cells colored per `STATUS_CONFIG` (amber for Elaboración, blue Propuesta, green Ganada, red Perdida, orange Pend. Facturar, teal Facturada)
- Acquisition cells colored (sky for Venta, pink for DaaS)
- Indigo header row, alternating gray rows, thin gray borders
- Summary row at bottom with sum of Subtotal and USD Est.
- Frozen header pane
- Filename: `Informe Forecast dd-mm-aaaa HH-mm-ss.xlsx` (Windows-safe)
...
**Changes:**
- Added `useState` import for `isExporting` state
- Added `FileSpreadsheet` icon from lucide-react
- Added `exportDashboardToExcel` import
- Added `handleExportExcel` async handler with loading state management
- Added green "Exportar Excel" button in the table header bar (next to record count), with spinner while exporting and disabled when no rows

---

### No changes needed to `useDashboard.ts`

`getSubtotalUsd` was already exported — verified at [line 89](file:///d:/novotechflow/apps/web/src/hooks/useDashboard.ts#L89).

## Verification

> [!NOTE]
> TypeScript verification via `tsc --noEmit` could not be run due to sandbox restrictions. Code follows the exact same patterns as the existing `exportExcel.ts` and uses only already-exported types/functions.

Manual verification needed: click the "Exportar Excel" button on the Dashboard to confirm the file downloads with proper formatting.
