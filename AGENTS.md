...
**Política de dilución (ADR-115):** la dilución reparte el costo ATERRIZADO Y CONVERTIDO, nunca el costo crudo. El aporte de cada item —el del diluido al total que se reparte, y el del visible al peso con que lo recibe— es su landed total vía `calculateItemLandedTotal`: costo convertido a la moneda del escenario + flete del padre + costo total de sus hijos. Numerador y denominador se miden con la MISMA regla, y es eso lo que hace que los pesos sumen 1 y el reparto se conserve (Σ `dilutionPerUnit` × cantidad = `totalDilutedCost`, y el libro de costos = landed de los visibles + costo diluido). Antes ambos lados usaban `convertCost(unitCost) × quantity`: el flete y los hijos de un item diluido se evaporaban de la cotización —plata regalada, proporcional al flete de lo que se diluya y marcada por el margen de los visibles— y un visible con flete o hijos pesaba menos de lo que cuesta, así que recibía menos dilución de la que le toca. `calculateDilutionPerUnit` conserva su fórmula; lo que cambió es que sus llamadores le pasan el `baseLandedCost` del visible y no su costo crudo.
