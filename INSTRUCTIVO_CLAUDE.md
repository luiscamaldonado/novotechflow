...
**GitHub vía CLI.** Claude Code opera GitHub con el CLI `gh` (instalado y autenticado como Luis). Las **lecturas** las ejecuta libremente, sin pedir aprobación: `gh run list`/`gh run view --log-failed` (runs de CI y sus errores), `gh pr list`/`gh pr view`, `gh api` de solo lectura. Uso típico: tras un push de Luis, verificar directamente el run de CI en vez de esperar la notificación. Las **escrituras** (crear PRs o issues, re-run de workflows, editar cualquier cosa en GitHub) solo con aprobación previa de Luis, por operación. El merge a `master` y el `push` siguen siendo exclusivamente de Luis. Existe un MCP oficial de GitHub; igual que con Railway, no se documenta como garantía — el CLI es la vía estable.
...
- **Un mensaje del chat = un solo prompt ejecutable.** Claude no entrega dos o más prompts en un mismo mensaje, ni siquiera si ambos son de solo lectura sobre la misma rama: el siguiente prompt se redacta cuando llega el resultado del anterior, porque ese resultado puede cambiarlo. Entregar varios prompts de una invita a correrlos de corrido y rompe el gate de un paso a la vez.
- **Subagentes/Workflow de Claude Code:** permitidos por defecto en tareas de lectura y verificación paralela (recon, greps, gates); la ejecución de ediciones y los commits siguen siendo secuenciales, un paso a la vez. El prompt lo declara explícito. Codificado al cierre de ADR-117.
