...
2. No hagas grep / codebase search global. Abre DECISIONS.md y localiza el ADR-NNN objetivo por su encabezado, que empieza con `## ADR-NNN` (el separador y título que siguen pueden variar). Explorar el resto del repo no está permitido.
...
4. Si el cambio es una adenda, insértala en la ubicación exacta indicada abajo (normalmente al final del cuerpo del ADR, antes de la sección `### Archivos`/`### Commits`/`### Pendientes` si existen, o si no al final del cuerpo, antes del encabezado `##` del siguiente ADR), sin alterar el texto previo del ADR. No introduzcas líneas `---`; separa la adenda del contenido previo solo con una línea en blanco.
