...
export class BillingProjectionsService {
   ...
   */
  private async generateProjectionCode(
    nomenclature: string,
    userId: string,
  ): Promise<string> {
  ...
  async create(userId: string, data: CreateBillingProjectionDto) {
    ...
    const projectionCode = await this.generateProjectionCode(
      user.nomenclature,
      userId,
    );
    ...
    return this.prisma.billingProjection.create({
      data: {
        ...
        status:
          (data.status as ProposalStatus) || ProposalStatus.PENDIENTE_FACTURAR,
        billingDate: data.billingDate ? new Date(data.billingDate) : null,
        acquisitionType: data.acquisitionType
          ? (data.acquisitionType as AcquisitionType)
          : undefined,
  ...
  async update(
    id: string,
    data: UpdateBillingProjectionDto,
    user: AuthenticatedUser,
  ) {
    const existing = await this.prisma.billingProjection.findUnique({
      where: { id },
    });
    if (!existing) throw new NotFoundException('Proyección no encontrada.');
    if (user.role !== 'ADMIN' && existing.userId !== user.id) {
      throw new ForbiddenException(
        'No tienes permiso para modificar esta proyección.',
      );
    }
    ...
    return this.prisma.billingProjection.update({
      ...
      data: {
        clientName: data.clientName
          ? data.clientName.trim().toUpperCase()
          : undefined,
  ...
  async delete(id: string, user: AuthenticatedUser) {
    ...
    if (user.role !== 'ADMIN' && existing.userId !== user.id) {
      throw new ForbiddenException(
        'No tienes permiso para eliminar esta proyección.',
      );
    }
