...
function mapChildOut(child: ChildItemRel): ExternalChildItemOut {
  ...
  return {
    ...
    tipo: pickSpecString(specs, 'tipo'),
    responsable: pickSpecString(specs, 'responsable'),
    ...
    supplierCompanyName: child.item.supplierCompany?.name ?? null,
    supplierContactName: child.item.supplierContact?.name ?? null,
    supplierContactPhone: child.item.supplierContact?.phone ?? null,
    supplierContactEmail: child.item.supplierContact?.email ?? null,
    isTaxable: child.item.isTaxable,
  ...
  private mapScenario(
    ...
    const items: ExternalItemOut[] = s.scenarioItems.map((si, idx) => {
      ...
      return {
        ...
        supplierCompanyName: si.item.supplierCompany?.name ?? null,
