...
export async function validateCsvFile(
  ...
  if (buffer.length > CSV_MAX_SIZE_BYTES) {
    await safeUnlink(file.path);
    throw new BadRequestException(
      'El archivo CSV excede el l\u00edmite de 401KB.',
    );
  }
  ...
  if (buffer.includes(0x00)) {
    await safeUnlink(file.path);
    throw new BadRequestException(
      'El archivo contiene datos binarios y no es un CSV v\u00e1lido.',
    );
  }
 ...
 */
export async function validateImageFileSize(
  file: Express.Multer.File,
): Promise<void> {
...
export function sanitizeFilenameCsv(originalname: string): string {
  ...
  if (!ALLOWED_CSV_EXTENSIONS.includes(ext)) {
    throw new BadRequestException(
      `Extensi\u00f3n ${ext} no permitida para CSV. Solo .csv`,
    );
  }
  return sanitized;
}
