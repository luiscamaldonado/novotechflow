...
export class SpecPrefillService {
    ...
    async extraer(tipo: PrefillStrategyType, input: PrefillInput): Promise<PrefillResponseDto> {
        const strategy = this.strategies.get(tipo);
        if (!strategy) {
            throw new BadRequestException(`Tipo de extracci\u00f3n no soportado: ${tipo}`);
        }
