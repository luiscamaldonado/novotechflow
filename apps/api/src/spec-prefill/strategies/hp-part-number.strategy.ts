...
export class HpPartNumberStrategy implements PrefillStrategy {
    ...
    async ejecutar(data: PrefillInput): Promise<PrefillResponseDto> {
        ...
        const [raw] = await this.gemini.generarJson(prompt, SPEC_SCHEMA_OBJECT, 0.1);
        if (!raw || typeof raw !== 'object') {
            throw new BadRequestException(`No se pudo extraer informaci\u00f3n para '${cleanPartNumber}'.`);
        }

        const producto = this.mapProducto(raw as Record<string, unknown>, cleanPartNumber);
    ...
    private async consultarHp(
        ...
        try {
            ...
            const resumen = resumenResponse.data?.Body?.EnhancedDescription?.trim() || SIN_RESUMEN;
            ...
            const detalle = axios.isAxiosError(error)
                ...
                : String(error);
            this.logger.error(`Fallo al consultar HP PartSurfer para '${partNumber}': ${detalle}`);
            throw new BadRequestException('No se pudo consultar HP PartSurfer en este momento.');
        }
    }
...
${textoFinal}`;
    ...
    private mapProducto(raw: Record<string, unknown>, partNumber: string): ProductoPrefillDto {
