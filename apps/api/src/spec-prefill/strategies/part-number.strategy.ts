...
export class PartNumberStrategy implements PrefillStrategy {
    ...
    async ejecutar(data: PrefillInput): Promise<PrefillResponseDto> {
        ...
        if (!partNumber || partNumber.length < MIN_PART_NUMBER_LENGTH) {
            throw new BadRequestException('El Part Number es inv\u00e1lido o demasiado corto.');
        }
        ...
        const [raw] = await this.gemini.generarJson(plan.prompt, SPEC_SCHEMA_OBJECT);
        if (!raw || typeof raw !== 'object') {
            throw new BadRequestException(`No se pudo extraer informaci\u00f3n para '${partNumber}'.`);
        }

        const producto = this.mapProducto(raw as Record<string, unknown>, partNumber, plan);
        return { productos: [producto] };
    }

    private planDell(partNumber: string): PlanExtraccion {
        return { prompt: buildPromptDell(partNumber), fabricante: 'Dell', source: 'PART_NUMBER' };
    }
    ...
    private async planLenovo(partNumber: string): Promise<PlanExtraccion> {
        ...
        return { prompt: buildPromptJson(datos), fabricante: 'Lenovo', source: fuente };
    }
    ...
    private mapProducto(
        ...
        const toCampo = (value: string): CampoPrefillDto => ({ value, source: plan.source });
        return {
