...
export class LenovoPsrefService {
    ...
    async obtenerDatos(partNumber: string): Promise<LenovoDatosCrudos> {
        ...
        try {
            ...
            if (!productKey) {
                throw new NotFoundException(`MT '${mt}' no encontrado en el men\u00fa de PSREF.`);
            }
            if (refrescoFresco) {
    ...
    private async consultarSmartFindConFallo(partNumber: string): Promise<LenovoDatosCrudos> {
        try {
            ...
            throw new BadRequestException(`El equipo '${partNumber}' no pudo ser localizado.`);
        }
    }

    private async asegurarCache(): Promise<boolean> {
        const expirado = !this.cachedMenu || Date.now() - this.cacheTimestamp > CACHE_TTL_MS;
        if (!expirado) {
            return false;
        }
    ...
    private async refrescarCache(): Promise<void> {
        const handshake = await axios.get(`${PSREF_BASE}/api/home/VersionTimestamp?t=${Date.now()}`, {
            headers: { 'User-Agent': USER_AGENT },
            timeout: HANDSHAKE_TIMEOUT_MS,
        });
        const cookies: string[] = handshake.headers['set-cookie'] ?? [];
        this.cachedCookie = cookies.map((cookie) => cookie.split(';')[0]).join('; ');
        ...
        const menuResponse = await axios.get(
            ...
            { headers: { 'User-Agent': USER_AGENT, Cookie: this.cachedCookie }, timeout: MENU_TIMEOUT_MS },
        );
    ...
    private extraerProductKey(record: Record<string, unknown>, mt: string): string {
    ...
    private async consultarMatriz(productKey: string, partNumber: string, mt: string): Promise<string> {
        const apiUrl =
        ...
        const targetRow = matrix.rows.find((row) => row[0]?.toUpperCase() === partNumber);
        if (!targetRow) {
            throw new NotFoundException(`Modelo '${partNumber}' no hallado en PSREF.`);
        }
    ...
    private async consultarSmartFind(partNumber: string): Promise<string> {
        ...
        if (!datos || datos.length < MIN_SMARTFIND_LENGTH || datos.includes('"data":[]')) {
