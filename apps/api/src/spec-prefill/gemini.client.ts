...
export class GeminiClient {
    ...
    private async postWithRetry(payload: unknown): Promise<GeminiResponse> {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                const isRetryable =
                    status === HTTP_SERVICE_UNAVAILABLE || status === HTTP_TOO_MANY_REQUESTS;
    ...
    private parseResponse(data: GeminiResponse): unknown[] {
        ...
        if (typeof rawText !== 'string') {
            throw new BadGatewayException('Respuesta de Gemini sin contenido de texto.');
        }
        ...
        try {
            ...
            throw new BadGatewayException('Error al parsear la respuesta JSON de Gemini.');
        }
