...
export class TrmService {
  ...
  private async fetchTrmValues() {
    ...
    try {
      const setIcapRes = await axios.post(
      ...
      if (setIcapRes.data?.data?.avg) {
        results.setIcapAverage = this.parseCurrencyString(
          setIcapRes.data.data.avg,
        );
      }
    } catch (e) {
      this.logger.error(
        `Error fetching SET-ICAP average: ${(e as Error).message}`,
      );
    }

    // 2. Wilkinson Spot Average (Scraping)
    try {
      const wilkinsonRes = await axios.get(
        'https://dolar.wilkinsonpc.com.co/dolar-hoy-spot-minuto-a-minuto/',
        {
          headers: {
            'User-Agent':
              'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
      ...
      const spotText = $('.display-5.fw-bold.text-dark.lh-1.my-1 span')
        .first()
        .text();
      if (spotText) {
      ...
      this.logger.error(
        `Error fetching Wilkinson spot: ${(e as Error).message}`,
      );
    }
